...
# coloring the score plot according to genotype
...
We now would like to assess how much information about the *genotype* is contained in each block. We therefore perform a univariate analysis. Since we have observed that there is also a variation according to *sex* in the datasets, we want to test the effects of *gene*, *sex* and their interaction simultaneously.
...
omicade4::plot.mcia(latliv.mcia, axes = 1:2,
                    phenovec = as.character(genotype.fc),
                    sample.lab = FALSE)
```
...
>
> * What about using the full metabolomics datasets (i.e., including also the non-annotated variables?)
>
> * Conclusion?
...
Let us first look at the relationships between the components of the blocks:

```{r block_splsda_scores}
...
```{r block_splsda_heatmap, fig.width=10, fig.height=10, ppi=300}
mixOmics::cimDiablo(latliv.blocksplsda, margins = c(10,20),
...
```{r graph_block_splsda_circos, fig.width = 10, fig.height = 10}
...
```{r}
perf.blocksplsda$MajorityVote.error.rate
```

* **Weighted Vote**:

```{r}
perf.blocksplsda$WeightedVote.error.rate
```
