...
## Question 8: Does the integration of the clinical and transcriptomics
## datasets bring better insight into the biological similarities between
## samples within the same treatment dose?
## Multi--block PLS-DA

res.block.plsda.lt.gene.clinin.time <- block.plsda(
  X = list(gene = data.lt.gene,
           clinic = data.lt.clinic),
  Y = liver.toxicity$treatment$Time.Group)

plotIndiv(res.block.plsda.lt.gene.clinin.time)
plotVar(res.block.plsda.lt.gene.clinin.time)

## Sparse Multi-block PLS-DA

list.keepX <- list(gene = rep(10, 2), clinic = rep(10,2))

res.block.splsda.lt.gene.clinin.time <- block.splsda(
  X = list(gene = data.lt.gene,
           clinic = data.lt.clinic),
  Y = liver.toxicity$treatment$Time.Group,
  keepX = list.keepX)

plotIndiv(res.block.splsda.lt.gene.clinin.time,
          title = "Full design, block: gene", 
          group=liver.toxicity$treatment$Time.Group,
          ind.names=FALSE, legend = TRUE)

plotVar(res.block.splsda.lt.gene.clinin.time)

cimDiablo(res.block.splsda.lt.gene.clinin.time,
          color.blocks = color.mixo(5:6),
          color.Y=color.mixo(c(2,3,4,1)))


## With another design

design.da <- matrix(c(0,0.1,1,0.1,0,1,1,1,0), ncol=3)

res.block.splsda.lt.design.gene.clinin.time <- block.splsda(
  X = list(gene = data.lt.gene,
           clinic = data.lt.clinic),
  Y = liver.toxicity$treatment$Time.Group,
  keepX = list.keepX, design=design.da)


plotIndiv(res.block.splsda.lt.design.gene.clinin.time,
          group=liver.toxicity$treatment$Time.Group,
          ind.names=FALSE, legend = TRUE)

plotIndiv(res.block.splsda.lt.design.gene.clinin.time,
          group=liver.toxicity$treatment$Dose.Group,
          ind.names=FALSE, legend = TRUE, blocks = 1)


cimDiablo(res.block.splsda.lt.design.gene.clinin.time,
          color.blocks = color.mixo(5:6),
          color.Y=color.mixo(c(2,3,4,1)))
